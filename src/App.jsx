...
${milestones.length ? `<h2>Jalons officiels</h2> ${[...milestones].sort((a,b)=>a.date.localeCompare(b.date)).map((m) => `<div class="milestone">
<b>${m.done ? ‘✓ ’ : ‘’}${m.title}</b> — ${formatDate(m.date)}
${(m.deliverables || []).map((d) => `<div class="sub-item ${d.done ? 'done' : 'pending'}">${d.done ? '✓' : '○'} ${d.label}</div>`).join(’’)}

</div>`).join('')}` : ''}

${docs.length ? `<h2>Notes de documentation</h2> ${[...docs].sort((a,b)=>a.date.localeCompare(b.date)).map((d) => `<div class="doc-entry">
...
<style>{`* { -webkit-tap-highlight-color: transparent; box-sizing: border-box; } body { margin: 0; background: ${C.bg}; } input[type=date]::-webkit-calendar-picker-indicator { filter: invert(0.4); } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-thumb { background: ${C.border}; border-radius: 2px; } select option { background: ${C.surface}; color: ${C.text}; } button:active { opacity: 0.75; } [contenteditable]:empty:before { content: "Prenez vos notes ici…"; color: ${C.muted}; } [contenteditable]:focus { border-color: ${C.accent}; }`}</style>
